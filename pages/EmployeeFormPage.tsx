...
const EmployeeFormPage: React.FC = () => {
  ...
  const handleResetPassword = () => {
    const newPass = prompt("Enter new authorized password for " + formData.username + ":");
    if (newPass) {
      const updated = { ...formData, password: newPass };
      setFormData(updated);
      if (isEdit) updateEmployee(updated as Employee);
      alert('Security credentials successfully updated in main registry.');
    }
  };
        ...
        <div className="p-10 lg:p-16">
          <div className="flex justify-between items-start mb-12">
            ...
            <div className="relative group">
              ...
              <button 
                ...
                className="absolute -bottom-2 -right-2 w-12 h-12 bg-slate-900 text-white rounded-2xl flex items-center justify-center shadow-2xl hover:bg-indigo-600 transition-colors"
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-6 py-4 bg-slate-50 border border-transparent rounded-[1.25rem] outline-none focus:ring-4 focus:ring-indigo-100 focus:bg-white focus:border-indigo-100 transition-all text-sm font-bold placeholder:text-slate-300"
                  placeholder="e.g. Johnathan S. Archer"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-6 py-4 bg-slate-50 border border-transparent rounded-[1.25rem] outline-none focus:ring-4 focus:ring-indigo-100 focus:bg-white focus:border-indigo-100 transition-all text-sm font-bold placeholder:text-slate-300"
                  placeholder="e.g. Flight Support Specialist"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-6 py-4 bg-slate-50 border border-transparent rounded-[1.25rem] outline-none focus:ring-4 focus:ring-indigo-100 focus:bg-white focus:border-indigo-100 transition-all text-sm font-bold placeholder:text-slate-300"
                  placeholder="u_jarcher"
                />
              </div>

              <div className="space-y-2 relative">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Authorization Token (Password)</label>
                <input
                  type="password" required={!isEdit} value={formData.password}
                  onChange={e => setFormData({ ...formData, password: e.target.value })}
                  className="w-full px-6 py-4 bg-slate-50 border border-transparent rounded-[1.25rem] outline-none focus:ring-4 focus:ring-amber-100 focus:bg-white focus:border-amber-100 transition-all text-sm font-bold placeholder:text-slate-300"
                  placeholder={isEdit ? "•••••••• (unchanged)" : "Secure Password"}
                />
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-6 p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100">
              <div className="flex items-center space-x-4">
                <div className="relative">
                  <input
                    ...
                    className="peer w-8 h-8 rounded-xl border-2 border-indigo-200 text-indigo-600 focus:ring-4 focus:ring-indigo-100 transition-all cursor-pointer appearance-none bg-white checked:bg-indigo-600 checked:border-indigo-600"
                  />
              ...
              {isEdit && (
                <button 
                  ...
                  className="px-6 py-3 bg-amber-500/10 text-amber-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-500 hover:text-white transition-all active:scale-95 shadow-lg shadow-amber-500/10"
                >
            ...
            <div className="pt-10 flex flex-col md:flex-row gap-6 items-center">
              <button 
                type="submit" 
                className="w-full md:w-auto md:flex-1 bg-slate-900 text-white font-black py-5 rounded-[2rem] shadow-2xl shadow-slate-900/20 hover:bg-indigo-600 hover:translate-y-[-4px] transition-all active:scale-95 uppercase tracking-[0.2em] text-sm"
              >
              ...
              <button 
                ...
                className="w-full md:w-64 py-5 text-slate-400 font-black hover:bg-slate-50 rounded-[2rem] transition-all uppercase tracking-[0.2em] text-sm"
              >
                Cancel Protocol
              </button>
            </div>
